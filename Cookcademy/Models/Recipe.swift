...
//The Recipe structure holds MainInfromation + ingredients + directions
struct Recipe: Identifiable, Codable {
    ...
    //initializer will be used for making empty recipes that the user can edit
    init() {
        self.mainInformation = MainInformation(name: "", description: "", author: "", category: .breakfast)
        self.ingredients = []
        self.directions = []
    }
}

//The MainInformation structure holds the key components of a recipe
...
// The Ingredient structure describes each ingredient
struct Ingredient: Identifiable, RecipeComponent {
    ...
    var description: String {
        ...
        switch unit {
            case .none:
            let formattedName: String = quantity == 1 ? "\(name)" : "\(name)s"
...
/* The Direction structure holds the contents of a direction in a recipe.
   isOptional property show that some info is additional */
